...
static int touch_int_complete_flag = 0;

#ifdef CONFIG_FW_DOWNLOAD_BOOT
static uint8_t mfg_flag = 0;
#endif
#ifdef CONFIG_SYNC_TOUCH_STATUS

static void switch_sensor_hub(struct himax_ts_data *ts, int mode);
int touch_status(u8 reg, u8 status);
int touch_get_status(u8 reg);
#endif
#define COMPARE_CTP_MODULE
#if defined(COMPARE_CTP_MODULE)
typedef enum hx852xe_ctp
{
        HX852XE_CTP_MAIN = 0,   //default module
        HX852XE_CTP_SECOND = 1,
}HX852XE_CTP_MODULE;

static HX852XE_CTP_MODULE hx852xe_ctp_module_name = HX852XE_CTP_MAIN;
#endif
...
#ifdef CONFIG_TOUCHSCREEN_TOUCH_FW_UPDATE
#define TOUCH_VENDOR "HM852XE"
#define FW_FLASH_TIMEOUT 200
static uint8_t g_fw_verion = 0;
struct touch_fwu_notifier himax_tp_notifier;
static int register_himax_touch_fw_update(void);
...
static void doFirmwareUpdate(void *unused, async_cookie_t cookie)
{
			...
			if(FirmwareUpdate_Count<3)
				{
					...
					I("[BOOT_FW_DOWNLOAD] %s: TP upgrade Error, Count: %d\n", __func__, FirmwareUpdate_Count);
					goto doFirmwareUpdate_Retry;
				}
			E("[BOOT_FW_DOWNLOAD] %s: TP upgrade error, line: %d\n", __func__, __LINE__);
			fw_update_complete = false;
		}
	else
		{
			I("[BOOT_FW_DOWNLOAD] %s: TP upgrade OK, line: %d\n", __func__, __LINE__);
	...
	I("[FW][BOOT_FW_DOWNLOAD]firmware flash process complete!\n");
}
...
static int updateFirmware(void *arg)
{
...
#if defined(COMPARE_CTP_MODULE)
	switch (hx852xe_ctp_module_name) {
			case 0: snprintf(fwname, sizeof(fwname), "tp_HMX852XE_MAIN.img");
					break;
			case 1: snprintf(fwname, sizeof(fwname), "tp_HMX852XE_SECOND.img");
					break;
			default:snprintf(fwname, sizeof(fwname), "tp_HMX852XE_MAIN.img");
					break;
	}
#endif

	/* Request firmware for update */
	ret = request_firmware(&ts->fw, fwname, &ts->client->dev);
	if (ts->fw == NULL) {
		I("[FW][BOOT_FW_DOWNLOAD] No firmware file, ignored firmware update\n");
		goto HMX_FW_REQUEST_FAILURE;
	} else if (ret) {
		D("[FW][BOOT_FW_DOWNLOAD]  Request_firmware failed, ret = %d\n", ret);
		goto HMX_FW_REQUEST_FAILURE;
	}
	...
	} else { /* In suspend mode */
		if (ts->pdata->powerOff3V3 && ts->pdata->power)
				ts->pdata->power(1);
	}

	snprintf(chip_fw, 10, "%02X", private_ts->vendor_fw_ver);

	/* Check F/W header */
	if (!strncmp(ts->fw->data, "TP_", 3)) {
		/* Header Pattern: TP_HMXXXXX_VV\n, where XXXX is not cared, VV is the version number */

    		ts->fw_data_start = strchr(ts->fw->data, '\n');
               ver_string = strchr(ts->fw_data_start -3, '_');
               if(ver_string != NULL){
    		    memcpy(dt_fw, ver_string + 1, ts->fw_data_start++ - ver_string -1);
                   ret = strncmp(chip_fw, dt_fw, 2);
                   himax_HW_reset(false,true);
                   checksumResult = himax_calculateChecksum(true);    
                   E("[FW][BOOT_FW_DOWNLOAD] checksumResult=0x%x\n",checksumResult);
                
                   if ((checksumResult)&&(ret == 0)){
                        E("[FW][BOOT_FW_DOWNLOAD]  firmware version is the same in chip %s than in file %s, ignore\n",
                            chip_fw, dt_fw);
                        i2c_himax_write_command(private_ts->client, 0x83, normalRetry);
                        msleep(30);
                        i2c_himax_write_command(private_ts->client, 0x81, normalRetry);
                        msleep(50);
                        goto HMX_FW_SAME_VER_IGNORE;
                   }
                }
                
		ts->fw_size = ts->fw->size - (ts->fw_data_start - ts->fw->data + 1);
	} else {
		I("[FW][BOOT_FW_DOWNLOAD]  no touch firmware header, force write\n");
	...
	if (wake_lock_active(&flash_wake_lock))
		I("[FW][BOOT_FW_DOWNLOAD]  wake lock successfully acquired!\n");
	else {
		E("[FW][BOOT_FW_DOWNLOAD]  failed to hold wake lock, give up.....\n");
...
HMX_FW_SAME_VER_IGNORE:
	...
	} else { /* Start at suspend mode, finsih in suspend mode */
		if (ts->pdata->powerOff3V3 && ts->pdata->power)
				ts->pdata->power(0);
	}
	atomic_set(&ts->in_flash, 0);
...
#ifdef HX_TP_SYS_DIAG
            x_channel = HX_RX_NUM; // X channel
   			y_channel = HX_TX_NUM; // Y channel
#endif
		  	I("[FW][CONFIG_FW_DOWNLOAD_BOOT] HX_MAX_PT=0x%x\n",HX_MAX_PT);
                              HX_MAX_PT=HIMAX_FINGER_NUM;
...
static int i_update_FW(void)
{
	...
	if ( private_ts->vendor_fw_ver < i_CTPM_FW[FW_VER_MAJ_FLASH_ADDR] )
		{
...
static int himax_loadSensorConfig(struct i2c_client *client, struct himax_i2c_platform_data *pdata)
{
...
#ifdef CONFIG_TOUCHSCREEN_TOUCH_FW_UPDATE
	g_fw_verion = private_ts->vendor_fw_ver;
#endif

#if defined(HX_LOADIN_CONFIG)||defined(HX_AUTO_UPDATE_CONFIG)
#ifdef CONFIG_OF
		I("%s, config_selected, %4p\n", __func__ ,config_selected);
		if(config_load == false)
			{
	...
	if((data[0] != 0x0F || data[1] != 0x53) && (!(pdata->probe_check_flag)))
	{
		//IC is abnormal
	...
	if((data[0] != 0x0F || data[1] != 0x53) && (pdata->probe_check_flag))
	{
		//IC is abnormal while probe
		E("[HimaxError] %s R36 Fail : R36[0]=%d,R36[1]=%d,R36 Counter=%d in probe\n",__func__,data[0],data[1],ESD_R36_FAIL);
	}
#endif
...
#ifdef CONFIG_SYNC_TOUCH_STATUS
		/* switch to host firstly before HW reset, maybe do not change back to hub, sometimes need to suspend and resume again. */
...
static bool himax_ic_package_check(struct himax_ts_data *ts)
{
	...
	void ESD_HW_REST(void)
	{
		...
		/*if(himax_loadSensorConfig(private_ts->client, private_ts->pdata)<0)
			...
			break;*/
		i2c_himax_write_command(private_ts->client, 0x83, HIMAX_I2C_RETRY_TIMES);
		msleep(30);
		i2c_himax_write_command(private_ts->client, 0x81, HIMAX_I2C_RETRY_TIMES);
		msleep(50);
		}
...
//#ifdef CONFIG_SYNC_TOUCH_STATUS
					touch_int_complete_flag = 0;			//leave int
//#endif
					return;
				}
		}
	...
	else
	{
			...
			for(i = 0; i < hx_touch_info_size; i++)
			{
				...
				else if(buf[i] == 0xEC)/*case 1 ESD recovery flow*/
				{
#endif
					check_sum_cal = 2;
				}
...
#ifdef CONFIG_SYNC_TOUCH_STATUS
				else if (check_sum_cal == 3) {
					I("[HIMAX TP MSG]: TOUCH EMPTY - ALL 0xED.\n");
					ESD_HW_REST();
				}
#endif
...
#ifndef CONFIG_SYNC_TOUCH_STATUS
	if (ts->debug_log_level & BIT(0)) {
		I("%s: raw data:\n", __func__);
		for (loop_i = 0; loop_i < hx_touch_info_size; loop_i++) {
			I("0x%2.2X ", buf[loop_i]);
					...
					for (loop_i = 0; loop_i < ts->nFinger_support && (ts->debug_log_level & BIT(3)) > 0; loop_i++) {
						...
						if (((ts->pre_finger_mask >> loop_i) & 1) == 1) {
							if (ts->useScreenRes) {
								...
								I("status:%X, Raw:F:%02d Up, X:%d, Y:%d, N:%d, TP FW:%#x:SensorId:%#x\n",0, loop_i+1, ts->pre_finger_data[loop_i][0],
                                    ts->pre_finger_data[loop_i][1], Last_EN_NoiseFilter,private_ts->vendor_fw_ver, private_ts->vendor_sensor_id);
							}
						}
					}
...
#ifdef CONFIG_SYNC_TOUCH_STATUS
static ssize_t touch_status_store(struct device *dev,
...
static DEVICE_ATTR(touch_status, (S_IWUSR|S_IRUGO), NULL, touch_status_store);
#endif
...
static ssize_t himax_loadconfig_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	ssize_t ret = 0;

	ret += sprintf(buf, "%s_CFG:%#x\n", HIMAX852xes_NAME, private_ts->vendor_config_ver);

	return ret;
}

static ssize_t himax_loadconfig_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	char buf_tmp[6], length = 0;
	unsigned long result	= 0;
	uint8_t loop_i			= 0;
	uint16_t base			= 5;
	uint8_t write_da[256];
	uint8_t register_command = 0;
	bool	config_bank_reg = false;
	const int normalRetry = 10;
	struct himax_ts_data *ts = private_ts;
	int ret = 0;
	uint8_t cmd[4];
	...
	if (buf[0] == '0' && buf[1] == 'x') {
		if ((buf[2] == '0' && buf[3] == '0') || (buf[2] == '9' && buf[3] == 'E')) {//Config bank register
			config_bank_reg = true;
		} else {
			config_bank_reg = false;
		}
		...
		for (loop_i = 0; loop_i < 256; loop_i++) {
			if (buf[base] == '\n') {
				if (config_bank_reg) {
					mutex_lock(&i2c_access);
					//Config Bank register
					himax_config_reg_write(private_ts->client, register_command, &write_da[0], length, normalRetry);
					msleep(10);
					mutex_unlock(&i2c_access);
					I("Config Bank register CMD: %x, %x, %d\n", register_command, write_da[0], length);
				} else {
					mutex_lock(&i2c_access);
					i2c_himax_write(private_ts->client, register_command, &write_da[0], length, normalRetry);
					msleep(5);
					mutex_unlock(&i2c_access);
					I("command CMD: %x, %x, %d\n", register_command, write_da[0], length);
				}
		...
		i2c_himax_read(private_ts->client, 0x37, cmd, 4, DEFAULT_RETRY_CNT);
		private_ts->vendor_config_ver = cmd[0];
		I("config_ver = %x\n", cmd[0]);
	}
	else {
...
static DEVICE_ATTR(loadconfig, (S_IWUSR|S_IRUGO), himax_loadconfig_show, himax_loadconfig_store);
#endif
...
static ssize_t himax_debug_dump(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if ( buf[0] == 'n') {
		...
		for (result=0;result<2000;result++) {
			i2c_himax_write_command(private_ts->client, 0x82, HIMAX_I2C_RETRY_TIMES);
			msleep(30);

			i2c_himax_write_command(private_ts->client, 0x80, HIMAX_I2C_RETRY_TIMES);
			msleep(50);
...
static int himax_touch_sysfs_init(void)
{
	...
	if (ret) {
		E("%s: sysfs_create_file dev_attr_touch_status failed\n", __func__);
		return ret;
	}
#endif
	...
	#endif
         #ifdef HX_TP_COMMAND
	ret = sysfs_create_file(android_touch_kobj, &dev_attr_loadconfig.attr);
	if (ret)  
	{    
		E("sysfs_create_file dev_attr_loadconfig failed\n");
		return ret;
	}    
         #endif    
...
static void himax_touch_sysfs_deinit(void)
{
	...
	sysfs_remove_file(android_touch_kobj, &dev_attr_register.attr);
         ...
         #ifdef HX_TP_COMMAND
        sysfs_remove_file(android_touch_kobj, &dev_attr_loadconfig.attr);
	#endif
...
static int himax_parse_config(struct himax_ts_data *ts, struct himax_config *pdata)
{
	...
	#if 1
		I(" DT#%d-def_cfg:%d,id:%05x, FW:%4x, len:%d,", i,
				...
				struct himax_i2c_platform_data *pdata)
{
...
#ifdef QCT
	pdata->gpio_irq = of_get_named_gpio(dt, "irq_gpio", 0);
	if (!gpio_is_valid(pdata->gpio_irq)) {
		I(" DT:gpio_irq value is not valid\n");
	}
	pdata->gpio_reset = of_get_named_gpio(dt, "rst_gpio", 0);
	if (!gpio_is_valid(pdata->gpio_reset)) {
		I(" DT:gpio_rst value is not valid\n");
	}
	pdata->gpio_3v3_en = of_get_named_gpio(dt, "3v3_gpio", 0);
	if (!gpio_is_valid(pdata->gpio_3v3_en)) {
		I(" DT:gpio_3v3_en value is not valid\n");
	}
	I(" QCT **DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d", pdata->gpio_irq, pdata->gpio_reset, pdata->gpio_3v3_en);
#endif
...
#ifdef CONFIG_SYNC_TOUCH_STATUS
	pdata->gpio_switch = GPIO_TP_I2C_SEL;
#endif
	I(" MTK DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d",
		(pdata->gpio_irq)&0x0fffffff, (pdata->gpio_reset)&0x0fffffff, pdata->gpio_3v3_en);
#endif
	...
	if (of_property_read_bool(dt, "switch_invert")) {
		I("switch pin invert");
		pdata->switch_invert = true;
	} else {
		I("No property: switch_invert");
		pdata->switch_invert = false;
	}
...
#if defined(COMPARE_CTP_MODULE)
	switch (hx852xe_ctp_module_name) {
			case 0: snprintf(namebuf, sizeof(namebuf), "MAIN");
					break;
			case 1: snprintf(namebuf, sizeof(namebuf), "SECOND");
					break;
...
//	himax_HW_reset(false, false);
	...
	himax_int_enable(private_ts->client->irq,0);
	wake_lock_init(&flash_wake_lock, WAKE_LOCK_SUSPEND, HIMAX852xes_NAME);
	wake_lock(&flash_wake_lock);
	if (wake_lock_active(&flash_wake_lock))
...
doFirmwareUpdate_Retry:
	I("%s: downloadtp upgrade start,len %d: %02X\n", __func__, FileLength, *(ts_fwu->fw_data_start));

	if (fts_ctpm_fw_upgrade_with_sys_fs(ts_fwu->fw_data_start, FileLength, true) == 0) {
...
//			touch_status(0);
#ifdef MTK
			if (ts->pdata->switch_invert)
				himax_switch_gpio_set(ts->pdata->gpio_switch, 1);
			else
				himax_switch_gpio_set(ts->pdata->gpio_switch, 0);
#else
			if (ts->pdata->switch_invert)
				gpio_direction_output(ts->pdata->gpio_switch, 1);
			else
				gpio_direction_output(ts->pdata->gpio_switch, 0);
#endif
			break;
		case 1:
#ifdef MTK
...
static void free_finger(struct himax_ts_data *ts)
{
	if (ts->finger_press) {
		I("free_finger\n");
		if (ts->protocol_type == PROTOCOL_TYPE_A) {
...
static int himax852xes_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef HX_MTK_DMA// Allocate the MTK's DMA memory
	...
	pdata->probe_check_flag = 1;
	if (himax_loadSensorConfig(client, pdata) < 0) {
		E("%s: Load Sesnsor configuration failed, unload driver.\n", __func__);
		goto err_detect_failed;
	}
	pdata->probe_check_flag = 0;
#if defined(COMPARE_CTP_MODULE)
	ts->vendor_sensor_id = himax_read_Sensor_ID(client);
	if (0x22 == ts->vendor_sensor_id) {	//the ctp module name is truly
		hx852xe_ctp_module_name = HX852XE_CTP_MAIN;
		I("himax, tpd_probe, hx852xe_ctp_module_name = HX852XE_CTP_MAIN\n");
	}
	else if(0x21 == ts->vendor_sensor_id) {// module name is JTOUCH
		hx852xe_ctp_module_name = HX852XE_CTP_SECOND;
		I("himax, tpd_probe, hx852xe_ctp_module_name = HX852XE_CTP_SECOND\n");
	}
	else if(0x12 == ts->vendor_sensor_id) {// module name is JTOUCH
		hx852xe_ctp_module_name = HX852XE_CTP_SECOND;
		I("himax, tpd_probe, hx852xe_ctp_module_name = HX852XE_CTP_SECOND\n");
       }
        else if(0x11 == ts->vendor_sensor_id) {// module name is JTOUCH
		hx852xe_ctp_module_name = HX852XE_CTP_SECOND;
		I("himax, tpd_probe, hx852xe_ctp_module_name = HX852XE_CTP_SECOND\n");
	}
       else if(0x11 == ts->vendor_sensor_id) {
		hx852xe_ctp_module_name = HX852XE_CTP_SECOND;
		I("himax, tpd_probe, hx852xe_ctp_module_name = HX852XE_CTP_SECOND\n");
	}
#endif
	...
	hallsensor_register_notifier(&hallsensor_status_handler);
		...
		goto err_register_interrupt_failed;
            ...
            if (private_ts->update_feature){
                        ...
                        I("[FW][BOOT_FW_DOWNLOAD]  skip firmware update since under %s mode.\n", htc_get_bootmode());
            } else {
                        I("[FW][BOOT_FW_DOWNLOAD]  HMX_FW_UPDATE.\n");
                        kthread_run(updateFirmware, (void *)ts, "HMX_FW_UPDATE");
                }
        }else{
                I("[FW][BOOT_FW_DOWNLOAD] no support update_feature.\n");
            }  
#endif
...
static int himax852xes_suspend(struct device *dev)
{
	...
	{
		ts->suspended = true;
		I("%s: enter <>FW:%#x:SensorId:%#x\n", __func__,private_ts->vendor_fw_ver, private_ts->vendor_sensor_id);
	}
...
//#ifdef CONFIG_SYNC_TOUCH_STATUS
	for(i=0; i <= 50; i++) {
		if(touch_int_complete_flag == 0) {
			I("%s, suspend wait for INT handle complete, i is %d\n", __FUNCTION__, i);
			break;
		}
		mdelay(10);
		if(i == 50) {
			E("%s, suspend wait for INT handle complete timeout i is %d\n", __FUNCTION__, i);
		}
	}
//#endif
...
//	msleep(50);
	...
	buf[1] = 0x40;
	ret = i2c_himax_master_write(ts->client, buf, 2, HIMAX_I2C_RETRY_TIMES);
	ret = i2c_himax_read(ts->client, 0xF4, &gesture_value, 1, DEFAULT_RETRY_CNT);
	I("[himax] %s: gesture value = 0x%x\n", __func__, gesture_value);

	/* switch to hub */
	switch_sensor_hub(ts, 1);
	/*set touch status, which means touch i2c to hub*/
	ret = touch_status(0x66, 1);
	if (ret == -1) {
		E("[himax]suspend,  has not sentral device\n");
	}
	mdelay(10);
#endif
...
//#ifdef CONFIG_SYNC_TOUCH_STATUS
	for(i=0; i <= 50; i++) {
		if(touch_int_complete_flag == 0) {
			I("%s, resume wait for INT handle complete, i is %d\n", __FUNCTION__, i);
			break;
		}
		mdelay(10);
		if(i == 50) {
			E("%s, resume wait for INT handle complete timeout i is %d\n", __FUNCTION__, i);
		}
	}
//#endif

#ifdef CONFIG_SYNC_TOUCH_STATUS
/*     set back touch status    */
	rc = touch_status(0x66, 0);
	if (rc == -1) {
		I("[himax]resume,  has not sentral device\n");
	}

// no need this step cause of HW reset when resume
/*
// get hub's touch status, 100ms's timeout

	touch_value = touch_get_status(0x4F);
	while ((touch_value&0x04)==0x04) {
		mdelay(5);
		// if touch_value bit2 is set, means hub side doing sense on&sleep out or sense off&sleep
		touch_value = touch_get_status(0x4F);
		i++;
		if (i > 10) {
				E("%s, checking timeout happened, hub side touch busy now!\n", __FUNCTION__);
				break;
		}
	}
	I("%s, retris %d , register 0x%2x status is 0x%2x\n", __FUNCTION__, i, 0x4F, touch_value);
*/
...
// no need this two steps cause of HW reset when resume
/*
//  sense off, sleep in  firstly
	buf1[0] = HX_CMD_TSSOFF;
	rc = i2c_himax_master_write(ts->client, buf1, 1, HIMAX_I2C_RETRY_TIMES);
	if (rc < 0) {
		E("[himax] %s: I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
	}
	msleep(30);

	buf1[0] = HX_CMD_TSSLPIN;
	rc = i2c_himax_master_write(ts->client, buf1, 1, HIMAX_I2C_RETRY_TIMES);
	if (rc < 0) {
		E("[himax] %s: I2C access failed addr = 0x%x\n", __func__, ts->client->addr);
	}
	msleep(50);
// enter normal mode of touch
	buf1[0] = 0xF4;
	buf1[1] = 0x00;
	rc = i2c_himax_master_write(ts->client, buf1, 2, HIMAX_I2C_RETRY_TIMES);
	i2c_himax_read(ts->client, 0xF4, &gesture_value, 1, DEFAULT_RETRY_CNT);
	I("[himax] %s: gesture value = %d\n", __func__, gesture_value);
*/
#endif
